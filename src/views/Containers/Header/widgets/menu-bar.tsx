      ...
      {menuData.map((menuItem, i) => {
        return (
          ...
          <li key={i} className={` ${menuItem.megaMenu ? "mega" : ""}`}>
            ...
            {menuItem.megaMenu && (
              <ul ref={ref} className={`mega-menu full-mega-menu submenu ${isComponentVisible && isOpen === menuItem.title ? "d-block" : ""} ${menuItem.title === "features" ? "feature-mega-menu" : ""}`}>
                {
                  <Container>
                    <Row>
                ...
                {menuItem.children.map((childrenItem: any, index: any) => (
                  <li key={index} className={`${childrenItem.children ? "submenuTitle " : ""}`}>
                    ...
                    {childrenItem.children && (
                      ...
                      ${path !== "/Layouts/layout3" && isComponentVisible && mobileSize && isSubNavOpen === childrenItem.title ? "d-block" : path === "/Layouts/layout3" && isComponentVisible && isSubNavOpen === childrenItem.title ? "d-block" : ""}`}
                      >
